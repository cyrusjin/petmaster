...
async function ensureOrdersCollection() {
  try {
    ...
    if (
      ...
      console.warn('createCollection orders skipped:', msg);
    }
  }
}
...
async function createOrder(event) {
  ...
  if (!merchantOpenid) {
    const { data: merchantUsers } = await users
      .where({ store_id: payload.store_id, isMerchant: true })
      .limit(1)
      .get();
  ...
  await orders.add({ data: orderData });
...
async function fetchPetsMap(petIds) {
  ...
  for (let i = 0; i < ids.length; i += 20) {
    ...
    const { data } = await pets.where({ pet_id: _.in(chunk) }).get();
    (data || []).forEach((doc) => {
      map[doc.pet_id] = doc;
    });
  }
...
async function listMerchantOrders(event) {
  const { OPENID } = cloud.getWXContext();
  if (!OPENID) return { success: false, errMsg: '无法获取用户身份' };
...
async function updateOrder(event) {
    ...
    if (confirmingPrice) {
      ...
      allowedFields.forEach((key) => {
        if (updates[key] !== undefined) {
          ...
          } else if (key === 'feeSnapshot') {
            patch[key] = updates[key];
          } else if (['boardingFee', 'shippingFee', 'totalFee', 'basePrice', 'days'].includes(key)) {
            patch[key] = updates[key];
          } else {
            patch[key] = updates[key];
          }
  ...
  await orders.doc(existing._id).update({ data: patch });
  let petDoc = null;
  if (existing.petId) {
  ...
  try {
    switch (event.action) {
      ...
      case 'listMerchantOrders':
        return await listMerchantOrders(event);
      case 'updateOrder':
    ...
    return {
      success: false,
      errMsg: (err && (err.errMsg || err.message)) || '订单服务执行失败'
    };
  }
};
